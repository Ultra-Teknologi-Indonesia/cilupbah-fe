...
export const PurchaseReturnService = {
  list: async (params: PurchaseReturnListParams = {}) => {
    ...
    if (params["filter[supplier_id]"])
      sp.set("filter[supplier_id]", params["filter[supplier_id]"]);
    ...
    const res = await fetchClient<ApiPaginated<PurchaseReturn>>(
      `/purchase/purchase-returns?${sp}`,
    );
    return { items: res.data ?? [], meta: res.meta };
  },

  getById: async (id: string) => {
    const res = await fetchClient<ApiResponse<PurchaseReturn>>(
  ...
  process: async (id: string, data: { processed_by: string }) => {
    const res = await fetchClient<ApiResponse<PurchaseReturn>>(
      `/purchase/purchase-returns/${id}/process`,
      {
        method: "POST",
        data,
      },
