...
export const StockOpnameService = {
  ...
  getItems: async (
    ...
    const res = await fetchClient<ApiPaginated<StockOpnameItem>>(
      `${BASE}/${id}/items?${sp}`,
    );
    return { items: res.data ?? [], meta: res.meta };
  },
  ...
  start: async (id: string, processBy: string) => {
    const res = await fetchClient<ApiResponse<StockOpname>>(
      `${BASE}/${id}/start`,
      {
        method: "POST",
  ...
  countItem: async (
    ...
    const res = await fetchClient<ApiResponse<StockOpnameItem>>(
      `${BASE}/${opnameId}/items/${itemId}/count`,
      {
        method: "POST",
        data,
      },
