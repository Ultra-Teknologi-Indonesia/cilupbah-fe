...
export const ReturnSettlementService = {
  list: async (limit = 200) => {
    const res = await fetchClient<ApiPaginated<ReturnSettlement>>(
      `${BASE}?limit=${limit}`,
    );
  ...
  getById: async (id: string) => {
    const res = await fetchClient<ApiResponse<ReturnSettlement>>(
      `${BASE}/${id}`,
    );
    return res.data;
  },
  ...
  create: async (data: { return_id: string; notes?: string }) => {
    const res = await fetchClient<ApiResponse<ReturnSettlement>>(BASE, {
      method: "POST",
      data,
    });
    return res.data;
  ...
  addRefund: async (data: CreateRefundPayload) => {
    const res = await fetchClient<ApiResponse<ReturnSettlementRefund>>(
      `${BASE}/refunds`,
      { method: "POST", data },
    );
    return res.data;
  ...
  addInvoiceDeduction: async (data: CreateInvoiceDeductionPayload) => {
    const res = await fetchClient<ApiResponse<unknown>>(`${BASE}/invoices`, {
      method: "POST",
      data,
    });
    return res.data;
