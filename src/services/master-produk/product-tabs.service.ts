...
export const ProductTabsService = {
  ...
  bulkVariants: async (
    ...
    ApiResponse<{ affected?: number; deleted?: number; blocked?: string[] }>
  > => {
    return fetchClient(`/products/${productId}/variants/bulk`, {
      method: "POST",
      data: body,
    });
  },
    ...
    return {
      items: (res.data ?? []).map((r) => ({
        ...
        listings: (r.listings ?? []).map((l) => ({
          ...
          externalSkuId: l.external_sku_id,
          channelUrl: l.channel_url,
