...
export const NaikkanService = {
  list: async (params: NaikkanListParams = {}) => {
    ...
    const res = await fetchClient<ApiPaginated<RawStore>>(
      `/raise-products?${q}`,
    );
  ...
  detail: async (id: string, params: NaikkanDetailParams = {}) => {
    const q = new URLSearchParams();
    if (params.search) q.set("search", params.search);
    if (params.filterIsActive !== undefined)
      q.set("filter[is_active]", String(params.filterIsActive));
    if (params.filterIsSuccess !== undefined)
      q.set("filter[is_success]", String(params.filterIsSuccess));
    q.set("page", String(params.page ?? 1));
    q.set("per_page", String(params.perPage ?? 25));

    const res = await fetchClient<
      ApiResponse<RawStore & { details: RawDetail[] }> & {
  ...
  history: async (id: string, params: NaikkanHistoryParams = {}) => {
    ...
    const res = await fetchClient<ApiPaginated<RawDetail>>(
      `/raise-products/${id}/history?${q}`,
    );
  ...
  destroy: async (raiseProductId: string) => {
    await fetchClient(`/raise-products/${raiseProductId}`, {
      method: "DELETE",
    });
  },
};
