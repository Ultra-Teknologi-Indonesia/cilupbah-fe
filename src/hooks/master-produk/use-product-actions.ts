...
export function useArchiveProduct() {
  ...
  return useMutation({
    mutationFn: ({ id, reason }: { id: string; reason?: string }) =>
      ProductDetailService.lifecycle(
        id,
        "archive",
...
export function useBulkArchive() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.failed > 0)
        toast.error(`${data.failed} produk gagal diarsipkan`, {
...
export function useBulkRestore() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.failed > 0)
        toast.error(`${data.failed} produk gagal dipulihkan`, {
          description: data.errors[0],
        });
      qc.invalidateQueries({ queryKey: LIST_KEY });
      qc.invalidateQueries({ queryKey: ARCHIVE_KEY });
...
export function useBulkDelete() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.failed > 0)
        toast.error(`${data.failed} produk gagal dihapus`, {
          description: data.errors[0],
        });
      qc.invalidateQueries({ queryKey: LIST_KEY });
    },
