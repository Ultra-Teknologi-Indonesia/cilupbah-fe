...
export function useCreateReturnSettlement() {
  ...
  return useMutation({
    mutationFn: (data: { return_id: string; notes?: string }) =>
      ReturnSettlementService.create(data),
    onSuccess: () => {
...
export function useAddRefund() {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (data: CreateRefundPayload) =>
      ReturnSettlementService.addRefund(data),
    onSuccess: () => {
...
export function useAddInvoiceDeduction() {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (data: CreateInvoiceDeductionPayload) =>
      ReturnSettlementService.addInvoiceDeduction(data),
    onSuccess: () => {
...
export function useRemoveInvoiceDeduction() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      ReturnSettlementService.removeInvoiceDeduction(id),
    onSuccess: () => {
