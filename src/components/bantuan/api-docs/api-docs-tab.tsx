...
export function ApiDocsTab() {
  ...
  const [activeModuleSlug, setActiveModuleSlug] = React.useState<string | null>(
    moduleFromUrl,
  );
  const [activeModule, setActiveModule] = React.useState<ApiDocModule | null>(
    null,
  );
  const [activeEndpointId, setActiveEndpointId] = React.useState<string | null>(
    null,
  );
  ...
  const moduleLoading =
    activeModuleSlug !== null && activeModule?.slug !== activeModuleSlug;
  ...
  const activeEndpoint =
    activeModule?.endpoints.find((e) => e.id === activeEndpointId) ?? null;
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-stretch gap-x-6 gap-y-3 divide-x divide-border/60 text-sm">
          ...
          <div className="pr-6 pl-0 sm:pl-6">
            ...
            <div className="text-lg font-semibold tabular-nums text-warning">
              {index.totals.undocumented}
            </div>
          </div>
          <div className="pl-0 sm:pl-6">
            <div className="text-xs text-muted-foreground">Digenerate</div>
            <div className="font-mono text-xs">
              {new Date(index.generated_at).toLocaleString("id-ID")}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[260px_320px_1fr]">
        <Card
          className={cn("flex flex-col gap-2 overflow-hidden p-3", panelHeight)}
        >
        ...
        {activeEndpoint ? (
          ...
          <Card
            className={cn("flex items-center justify-center p-8", panelHeight)}
          >
            <EmptyState
              title="Pilih endpoint"
              description="Pilih endpoint dari kolom kiri."
            />
          </Card>
        )}
      </div>
