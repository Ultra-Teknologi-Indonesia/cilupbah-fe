...
import { usePurchaseBills, useDeletePurchaseBill } from "@/hooks/transaksi-pembelian/use-purchase-bills"
import { useLocations } from "@/hooks/manajemen-rak/use-locations"
import type { PurchaseBill, PurchaseBillListParams, PurchaseBillStatus } from "@/types/transaksi-pembelian/purchase-bill"
...
export function TagihanListView() {
  ...
  const params = useMemo<PurchaseBillListParams>(() => ({
  ...
  const { data, isLoading, isFetching } = usePurchaseBills(params)
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-end">
        <Button variant="primary" asChild>
          <Link href="/dashboard/transaksi-pembelian/tagihan/tambah">
      ...
      <LiquidGlass radius={20} intensity="subtle" className="bg-white/30 dark:bg-white/[0.04]">
        <FilterToolbar
          ...
          searchPlaceholder="Cari no. tagihan, pemasok..."
          align="end"
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-3 py-20 text-muted-foreground">
              ...
              <div className="text-center">
                <p className="text-sm font-medium">Belum ada tagihan pembelian</p>
                <p className="mt-1 text-xs">Tambah tagihan baru untuk mencatat penagihan dari pemasok.</p>
              </div>
            </div>
          ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/60 bg-muted/30">
                      {["No. Tagihan", "No. Pesanan", "Pemasok", "Lokasi", "Tgl. Tagihan", "Status", "Nilai", "Dibayar", "Sisa", ""].map((h) => (
                      ...
                      return (
                        <tr key={item.id} className="border-b border-border/20 transition-colors last:border-0 hover:bg-muted/40">
                          <td className="whitespace-nowrap px-3 py-3 font-medium">
                            <Link href={`/dashboard/transaksi-pembelian/tagihan/${item.id}`} className="hover:text-primary hover:underline">
                              {item.bill_number}
                            </Link>
                          </td>
                          <td className="whitespace-nowrap px-3 py-3 text-muted-foreground">
                            {item.purchase_order ? (
                              <Link href={`/dashboard/transaksi-pembelian/pesanan/${item.purchase_order.id}`} className="hover:text-primary hover:underline">
                          ...
                          <td className={cn("whitespace-nowrap px-3 py-3 text-right font-medium tabular-nums", remaining > 0 ? "text-amber-600" : "text-emerald-600")}>
      ...
      <ConfirmDialog
        ...
        title="Hapus Tagihan"
        description={`Apakah Anda yakin ingin menghapus tagihan "${deleteTarget?.bill_number}"? Tindakan ini tidak dapat dibatalkan.`}
        confirmLabel="Hapus"
        variant="destructive"
