...
export function OrderImportDialog({ open, onOpenChange, onQueued }: Props) {
  const inputRef = React.useRef<HTMLInputElement>(null);
  const [file, setFile] = React.useState<File | null>(null);
  const [dragOver, setDragOver] = React.useState(false);
  const [sizeError, setSizeError] = React.useState(false);

  const importFile = useOrderImportFile();
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Import Pesanan</DialogTitle>
          <DialogDescription>
            Upload file Excel berisi data pesanan. Download template terlebih
            dahulu.
          </DialogDescription>
        </DialogHeader>
