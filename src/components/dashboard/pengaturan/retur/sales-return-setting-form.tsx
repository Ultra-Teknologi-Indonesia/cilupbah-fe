...
import {
  useSalesReturnSetting,
  useSaveSalesReturnSetting,
} from "@/hooks/barang-masuk/use-sales-return-setting";
...
function SettingFormBody({ initial }: { initial: SalesReturnSetting }) {
  ...
  const [restockLocation, setRestockLocation] = useState(
    initial.default_restock_location_id ?? "",
  );
  const [validityDays, setValidityDays] = useState(
    initial.return_validity_days != null
      ? String(initial.return_validity_days)
      : "",
  );
  const [conditions, setConditions] = useState(
    (initial.allowed_conditions ?? []).join(", "),
  );
  const [refundMethods, setRefundMethods] = useState(
    (initial.allowed_refund_methods ?? []).join(", "),
  );
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="flex flex-col divide-y divide-border/60 px-5 py-2">
          <SettingRow
            title="Auto-terima retur (auto accept)"
            desc="Retur baru langsung disetujui otomatis tanpa review manual."
          >
            <Switch checked={autoAccept} onCheckedChange={setAutoAccept} />
          </SettingRow>
          <SettingRow
            title="Auto-restock (auto receive)"
            desc="Saat retur disetujui, barang langsung diterima kembali ke stok (tanpa langkah terima manual)."
          >
      ...
      >
        <div className="grid grid-cols-1 gap-4 px-5 py-5 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-sm font-medium">
              Metode Refund Diizinkan
            </Label>
            <Input
              value={refundMethods}
              onChange={(e) => setRefundMethods(e.target.value)}
              placeholder="cash, transfer, store_credit"
            />
            <p className="text-xs text-muted-foreground">
              Pisahkan dengan koma.
            </p>
...
export function SalesReturnSettingForm() {
  ...
  return (
    <SettingFormBody key={setting.updated_at ?? "loaded"} initial={setting} />
  );
}
