...
export function PickingDetailView({ id }: { id: string }) {
  ...
  const [activePick, setActivePick] = React.useState<{
    itemId: string;
    remaining: number;
  } | null>(null);
  ...
  const refocusScan = React.useCallback(
  ...
  const savePick = React.useCallback(
    (itemId: string, qtyAbsolute: number, binCode: string) => {
      pickItem.mutate(
        ...
        {
          onSuccess: () => {
            toast.success("Qty pick tersimpan.");
            playScanFeedback("ok");
          },
          onError: (e) => {
            toast.error(errMsg(e, "Gagal menyimpan pick."));
  ...
  const interceptCode = React.useCallback(
    (raw: string): boolean => {
      ...
      toast.error(
        `Kode "${raw}" tidak dikenal (bukan rak/SKU di picklist ini).`,
      );
      playScanFeedback("error");
      return false;
    },
  ...
  const handleFail = () => {
    const reason =
      window.prompt("Alasan gagal picking (opsional):") ?? undefined;
    failPicklist.mutate(
      { id, reason },
      {
        onSuccess: () => {
  ...
  const activeItem = activePick
    ? items.find((i) => i.id === activePick.itemId)
    : null;
            ...
            <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    startPicklist.mutate(id, {
                      ...
                      onError: (e) =>
                        toast.error(errMsg(e, "Gagal memulai picking.")),
                    })
                  }
              ...
              {editable && (
                <Button
                  ...
                  disabled={!allPicked || completePicklist.isPending}
                >
          ...
          {editable && (
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                {activeRack ? (
                  <span>
                    Rak aktif: <b className="font-mono">{activeRack.code}</b> —
                    scan SKU untuk pick.
                  </span>
                ) : (
                  <span>
                    Scan <b>rak</b> dulu sebelum scan produk.
                  </span>
                )}
              </div>
              ...
              {activePick && activeItem && (
                <div className="flex flex-wrap items-end gap-3 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Sisa {activePick.remaining} · rak {activeRack?.code}
                    </div>
                  </div>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item) => {
                    ...
                    return (
                      <TableRow
                        key={item.id}
                        className={cn(done && "bg-emerald-500/[0.04]")}
                      >
                        <TableCell>
                        ...
                        <TableCell>
                          {done ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                              <ScanBarcodeIcon className="size-3.5" /> Kurang{" "}
