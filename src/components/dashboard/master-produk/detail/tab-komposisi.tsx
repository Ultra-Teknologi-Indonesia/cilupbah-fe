...
export function TabKomposisi({
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border/60 bg-card/40 px-6 py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          Bundle ini belum memiliki produk penyusun.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Table
        className="min-w-[640px] border-collapse"
      ...
      >
        <TableHeader>
          <TableRow className="border-b border-border/60 bg-muted/40 text-left text-xs text-muted-foreground">
            <TableHead className="px-3 py-2.5 text-xs font-normal text-muted-foreground">
              Produk komponen
            </TableHead>
            <TableHead className="px-3 py-2.5 text-xs font-normal text-muted-foreground">
              SKU
            </TableHead>
            <TableHead className="px-3 py-2.5 text-center text-xs font-normal text-muted-foreground">
              Qty
            </TableHead>
            <TableHead className="px-3 py-2.5 text-right text-xs font-normal text-muted-foreground">
              Stok tersedia
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {components.map((c) => (
            ...
            >
              ...
              <TableCell className="px-3 py-2.5 font-mono text-xs text-primary">
                {c.sku ?? "—"}
              </TableCell>
              ...
              <TableCell className="px-3 py-2.5 text-right tabular-nums">
                {c.stock ? (
                  <span
                    ...
                    title={
                      c.stock.available < c.qty
                        ? "Kurang dari kebutuhan 1 bundle"
                        : undefined
                    }
                  >
                    {c.stock.available}
