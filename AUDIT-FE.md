...
| 🔶 | **§1.2 prefetch RSC** | **10 halaman** kini pakai pola `getServerQueryClient` + `HydrationBoundary` (dari 5): + `posisi-stok`, `monitor-stok`, `lokasi`, `pengaturan/pengguna`, `integrasi-channel`. Query key initial disamakan persis per view. **Sisa:** 4 halaman ber-tab (`kontak-pelanggan`, `transaksi-stok`, `barang-masuk`, `barang-keluar`) — butuh split server-page + client tab-shell, dikerjakan terpisah. |
| ✅ | **§2 layering service→komponen** | Selesai. **0 komponen** mengimpor `@/services/*` (dari 39). Semua akses data lewat hooks `src/hooks/**`; tipe/helper yang dibutuhkan komponen di-re-export dari layer hook. Hook baru dibuat untuk panggilan yang belum ada (mis. `useLogin/useLogout`, `usePutawayBins`, `useChannelSearchMulti`, `useNaikkanProductPicker`, prefetch helpers). `doc-actions.ts` dipindah implementasinya ke `hooks/proses-pesanan/use-doc-actions.ts` (file lama jadi re-export). Aturan ESLint `no-restricted-imports` tetap sebagai penjaga regresi. |
